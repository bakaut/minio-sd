...
mc --config-dir=/home/minio/.mc/ config host add current https://`hostname` $MINIO_ACCESS_KEY $MINIO_SECRET_KEY
chown -R minio:consul /home/minio/.mc/

systemctl daemon-reload && systemctl enable monitoring && systemctl start monitoring && systemctl status monitoring

echo "* * * * * root su - minio -s /bin/bash -c \"mc admin  info  server current --json | grep `hostname` > /home/minio/.minio/mon.json\"" >> /etc/crontab
